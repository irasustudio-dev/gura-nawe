          ...
          <div className="lg:col-span-5">
             <SectionHeader 
              ...
              <motion.div 
                ...
                viewport={{ once: true }}
                className="p-8 rounded-[40px] bg-violet-600 text-white relative overflow-hidden"
              >
           ...
           <div className="lg:col-span-7">
             <motion.div 
               ...
               whileInView={{ opacity: 1, y: 0 }}
               viewport={{ once: true }}
               className="bg-white dark:bg-slate-900 rounded-[48px] p-10 md:p-16 border border-slate-100 dark:border-slate-800 shadow-2xl relative overflow-hidden"
             >
                ...
                <form className="space-y-6 relative z-10" onSubmit={handleSubmit}>
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div className="space-y-2">
                       ...
                       <input 
                         ...
                         className={`w-full bg-slate-50 dark:bg-slate-950 border py-4 px-6 rounded-2xl focus:outline-none focus:ring-2 transition-all font-medium text-slate-900 dark:text-white ${
                           errors.name 
                            ? 'border-red-500 focus:ring-red-500/20 focus:border-red-500'
                            : 'border-slate-100 dark:border-slate-800 focus:ring-violet-500/20 focus:border-violet-500'
                         }`}
                       />
                       {errors.name && (
                   ...
                   <div className="space-y-2">
                     ...
                     <select 
                       ...
                       onChange={handleInputChange}
                       className={`w-full bg-slate-50 dark:bg-slate-950 border py-4 px-6 rounded-2xl focus:outline-none focus:ring-2 transition-all font-medium text-slate-900 dark:text-white appearance-none ${
                         errors.subject
                          ? 'border-red-500 focus:ring-red-500/20 focus:border-red-500'
                          : 'border-slate-100 dark:border-slate-800 focus:ring-violet-500/20 focus:border-violet-500'
                       }`}
                   ...
                   <div className="space-y-2">
                     ...
                     <textarea 
                       ...
                       className={`w-full bg-slate-50 dark:bg-slate-950 border py-4 px-6 rounded-2xl focus:outline-none focus:ring-2 transition-all font-medium text-slate-900 dark:text-white resize-none ${
                         errors.message
                          ? 'border-red-500 focus:ring-red-500/20 focus:border-red-500'
                          : 'border-slate-100 dark:border-slate-800 focus:ring-violet-500/20 focus:border-violet-500'
                       }`}
                   ...
                   <button 
                     type="submit"
                     disabled={isLoading}
                     className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-5 rounded-[24px] font-black text-xl flex items-center justify-center gap-3 shadow-2xl transition-all hover:bg-violet-600 dark:hover:bg-violet-500 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed group active:scale-[0.98]"
                   >
                     {isLoading ? (
                       <>
             ...
             >
                <a href={store.socialLinks.instagram} target="_blank" rel="noreferrer" className="hover:text-violet-600 transition-colors"><Instagram size={28} /></a>
                <a href={store.socialLinks.facebook} target="_blank" rel="noreferrer" className="hover:text-violet-600 transition-colors"><Facebook size={28} /></a>
